...
function unwrapJsonCodeFence(result) {
...
function moveAlias(value, target, aliases) {
...
function normalizeDataFlowNode(value, forcedKind) {
    if (typeof value !== 'object' || value === null || Array.isArray(value)) {
        return value;
    }
...
function normalizeDataFlow(value) {
    if (typeof value !== 'object' || value === null || Array.isArray(value)) {
        return value;
    }
    const normalized = { ...value };
...
function normalizeReportBody(result) {
    try {
...
class ReviewService {
    ...
    async runAgent(model, messages, rootPath, token, callbacks, validateFinal) {
        let inputTokens = 0;
        let outputTokens = 0;
        ...
        for (let round = 0; round <= maxAgentToolRounds; round += 1) {
            ...
            const response = await model.sendRequest(messages, canUseTools ? {
                tools: [...agentTools_1.reviewAgentTools],
            ...
            for (const call of toolCalls) {
                ...
                try {
                    const value = await (0, agentTools_1.invokeReviewAgentTool)(rootPath, call, filePath => accessedFiles.add(filePath));
    ...
    async review(model, skill, context, rootPath, token, onFragment, onToolCall, userFocus) {
        ...
        return this.runAgent(model, messages, rootPath, token, { onFragment, onToolCall }, reportJson_1.validateReviewReportJson);
    }
    async continueConversation(model, skill, history, prompt, rootPath, token, callbacks = {}) {
