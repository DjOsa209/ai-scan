export const reportJsonSchemaVersion = '2.0' as const;

export type FindingSeverity = 'critical' | 'high' | 'medium' | 'low';
export type FindingConfidence = 'high' | 'medium' | 'low';
export type ReportResult = 'pass' | 'findings' | 'incomplete';
...
export type DataFlowAnalysisMethod = 'ai-context' | 'ast-assisted';
export type DataFlowNodeKind = 'source' | 'propagator' | 'sink';

export interface ReviewDataFlowNode extends ReviewLocation {
...
export interface ReviewDataFlow {
	readonly analysisMethod: DataFlowAnalysisMethod;
	readonly nodes: readonly ReviewDataFlowNode[];
...
export interface ReviewFinding {
	...
	readonly dataFlow?: ReviewDataFlow;
}
...
export interface ReviewReportModel {
	readonly schemaVersion: typeof reportJsonSchemaVersion;
	readonly metadata: {
	...
	readonly counts: Readonly<Record<FindingSeverity | 'manualReview', number>>;
...
const results = new Set<ReportResult>(['pass', 'findings', 'incomplete']);
...
function exactKeys(value: Record<string, unknown>, name: string, allowed: readonly string[]): void {
...
function parseLocation(value: unknown, name: string): ReviewLocation {
...
function parseDataFlow(value: unknown, name: string): ReviewDataFlow {
	const dataFlow = record(value, name);
	exactKeys(dataFlow, name, ['analysisMethod', 'nodes', 'limitations']);
	const analysisMethod = text(dataFlow.analysisMethod, `${name}.analysisMethod`) as DataFlowAnalysisMethod;
	...
	const nodes = dataFlow.nodes.map((value, index) => {
		...
		const kind = text(node.kind, `${nodeName}.kind`) as DataFlowNodeKind;
...
function parseFinding(value: unknown, index: number): ReviewFinding {
	...
	const severity = text(finding.severity, `findings[${index}].severity`) as FindingSeverity;
	const confidence = text(finding.confidence, `findings[${index}].confidence`) as FindingConfidence;
...
export function parseReviewReportJson(reportJson: string): ReviewReportModel {
	let decoded: unknown;
	try {
	...
	if (report.schemaVersion !== reportJsonSchemaVersion) {
		throw new Error(`安全报告 schemaVersion 必须为 ${reportJsonSchemaVersion}`);
	}
	...
	const result = text(report.result, 'result') as ReportResult;
	...
	return {
		schemaVersion: reportJsonSchemaVersion,
		metadata: {
...
export function validateReviewReportJson(reportJson: string): void {
	parseReviewReportJson(reportJson);
}

export function reportSourcePaths(report: ReviewReportModel): string[] {
...
export function renderReviewReportMarkdown(report: ReviewReportModel): string {
