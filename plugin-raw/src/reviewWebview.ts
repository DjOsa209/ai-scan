...
import type { PlatformScanStatus } from './platformScan';
import type { ReviewReportArtifact } from './reportArtifact';
import { parseReviewReportJson, renderReviewReportMarkdown, ReviewReportModel } from './reportJson';

export type ReviewUiStatus = 'ready' | 'running' | 'complete' | 'error';

interface TimelineItem {
	readonly label: string;
	readonly message: string;
	readonly progress: number;
	readonly state: 'complete' | 'active' | 'pending' | 'error';
}
...
export interface ReviewUiState {
	...
	readonly report: ReviewReportModel;
	readonly scanHistory: {
		readonly selectedReportId: string;
		readonly generatedAt: string;
		readonly previousGeneratedAt: string;
		readonly previousCounts?: ReviewReportModel['counts'];
...
export interface ReviewHistoryEntry {
	...
	readonly generatedAt: string;
	readonly taskId: string;
	readonly counts: ReviewReportModel['counts'];
}

const emptyReport: ReviewReportModel = {
...
export class ReviewWebviewProvider implements vscode.WebviewViewProvider, vscode.Disposable {
	private view: vscode.WebviewView | undefined;
	private readonly stateChangeEmitter = new vscode.EventEmitter<ReviewUiState>();
	private state: ReviewUiState = {
		status: 'ready',
		detail: '等待开始安全扫描',
		progress: 0,
		stage: '准备就绪',
	...
	resolveWebviewView(view: vscode.WebviewView): void {
		...
		this.disposables.push(view.webview.onDidReceiveMessage(async (message: unknown) => {
			...
			const typedMessage = message as { type: string; location?: unknown };
	...
	begin(workspaceLabel: string): void {
		this.state = {
			...this.state,
			status: 'running',
			detail: '正在初始化安全扫描',
	...
	setStage(stage: { status: PlatformScanStatus; label: string; progress: number; message: string }, taskId: string): void {
		const timeline: TimelineItem[] = this.state.timeline.map(item => ({ ...item, state: item.state === 'active' ? 'complete' : item.state }));
		const existingIndex = timeline.findIndex(item => item.label === stage.label);
		const item: TimelineItem = { label: stage.label, message: stage.message, progress: stage.progress, state: 'active' };
		if (existingIndex >= 0) {
	...
	setStatus(status: ReviewUiStatus, detail: string): void {
		this.state = { ...this.state, status, detail };
		this.pushState();
	}

	setReport(artifact: ReviewReportArtifact, taskId = this.state.taskId, options?: {
		readonly previousReport?: ReviewReportModel;
		readonly previousGeneratedAt?: string;
		readonly storedLocally?: boolean;
		readonly restored?: boolean;
		readonly historyEntries?: readonly ReviewHistoryEntry[];
	}): void {
		...
		this.state = {
			...
			taskId,
			hasResult: true,
			reportMarkdown: renderReviewReportMarkdown(report),
			report,
			scanHistory: {
			...
			timeline: this.state.timeline.map(item => ({ ...item, state: 'complete' as const })),
		};
		this.pushState();
	}
