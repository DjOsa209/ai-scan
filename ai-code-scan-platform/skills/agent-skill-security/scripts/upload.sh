...
curl -fsS -X POST -H "Authorization: Bearer $SECURITY_PLATFORM_API_KEY" -H "Accept: application/json" -F "archive=@$archive;type=application/zip" "${SECURITY_PLATFORM_URL%/}/api/v1/agent-skill-scans/archive"